...
export default function PDFLexicon({frequency, data}) {
  ...
  const xTabLex = page.margin.left + 0;
  const xTabFreq = page.margin.left + 25;
  ...
  let dataByChap = data.reduce((acc, word) => {
    let firstLetter = word.lex[0].toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")
    if (acc[firstLetter]) {
  ...
  let writeWord = (word, y, colNb, writeVerseNb) => {
    ...
    doc
      .setFontSize(9)
      .text(`(${word.countInBook}|${word.freq})`, columnOffset + xTabFreq, y);
  ...
  for (let currentFirstLetter of Object.keys(dataByChap)) {
    let dataToWrite = dataByChap[currentFirstLetter];
    ...
    writeFirstLetter(currentFirstLetter, currentY);
    currentY += 10;
    topColumnY = currentY;
