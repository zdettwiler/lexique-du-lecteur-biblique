...
function Lexicon({ frequency, data }) {
  const [showLexiconCorrectionModal, setShowLexiconCorrectionModal] =
  ...
  const [validatedCorrectionForm, setValidatedCorrectionForm] =
    React.useState(false)
  ...
  const handleCloseLexiconCorrectionModal = () =>
    setShowLexiconCorrectionModal(false)
  ...
  const styleLang = data[0].strong[0] === 'G' ? styles.lexNT : styles.lexOT
    ...
    <>
      ...
      >
        <Modal.Header closeButton onHide={handleCloseLexiconCorrectionModal}>
          <Modal.Title id='contained-modal-title-vcenter'>
            Proposer une modification du mot{' '}
            <span className='lex'>{correctingWord.lex}</span> (
            {correctingWord.strong})
          </Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body>
          {correctionStatus === 201 && (
            <Alert variant='success'>
              <i className='bi bi-check2' /> Bien reçu! Merci pour votre
              contribution au LLB!
            </Alert>
          )}
        ...
        {!correctionStatus && (
          <Modal.Footer>
            ...
            <Button
              ...
              onClick={handleCloseLexiconCorrectionModal}
            >
              Annuler
            </Button>
          </Modal.Footer>
      ...
      <Container className={styles.lexicon}>
        ...
        {data.map((word, id, data) => {
          ...
          const verseIndicator =
            prevVerse !== word.verse ? (
          ...
          return (
            <div key={id}>
              ...
              <div className={styles.wordEntry}>
                ...
                <div
                  className={styleLang}
                  onClick={() => handleShowLexiconCorrectionModal(word)}
                >
                  <span>{word.lex}</span>
                </div>
