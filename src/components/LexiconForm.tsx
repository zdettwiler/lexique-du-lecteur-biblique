...
const occurenceValues = occurenceOptions.map((o) => o.value) as [
  string,
  ...string[]
]
  ...
  useEffect(() => {
    ...
    if (!storedForm || (book && chapter && occurences)) {
      localStorage.setItem(
        'lexicon-form',
        JSON.stringify({ book, chapter, occurences })
      )
      return
    }
    try {
      ...
      reset({
        ...
        chapter: Number(parsedStoredForm.chapter) ?? 1,
    ...
    <div className="font-sans w-full lg:w-[850px] sm:w-5/6 md: mx-auto mt-10 p-5">
      <Form {...form}>
        <form
          ...
          className="box-border lg:space-x-5 flex flex-wrap lg:flex-nowrap lg:items-end flex-col md:flex-row"
        >
          ...
          <Button
            type="submit"
            size="sm"
            className="font-sans font-medium grow-0 mx-auto mt-4 mb-4"
          >
            Générer le lexique
          </Button>
        </form>
