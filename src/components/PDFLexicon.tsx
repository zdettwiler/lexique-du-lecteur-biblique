...
export default function PDFLexicon({ book, chapters, occurences, link=false }: Props) {
  ...
  const fetchPDF = async () => {
    ...
    try {
      ...
      console.log(`Finished fetching PDF in ${Math.round(0.001 * (Date.now()-start))} s`)
      setLoading(false)
    }
  }
  ...
  if (!pdfUrl && !link && isLoading) {
    ...
    return (
      <Button variant='outline' size='icon-sm' className='font-sans' onClick={fetchPDF}>
        <FileText />
      </Button>
    )
    ...
    return (
      <a href={pdfUrl} download={`${book} ${chapters} (<${occurences}×) - Lexique du lecteur biblique.pdf`}>
        <Button variant='outline' size='icon-sm' className='font-sans'>
          <Download />
        </Button>
      </a>
